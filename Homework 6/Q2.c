...
//   *  The free slots are linked with a singly linked list (we call this a free list). |next| points to the next free slot.
...
void* my_malloc(size_t size) {
    ...
    simple_metadata_t* metadata = simple_heap.free_head;
    simple_metadata_t* prev = NULL;
    simple_metadata_t* temp_metadata = NULL;
    simple_metadata_t* temp_prev = NULL;
    int best_fit_flag = 0;
    
    // To find a free slot.
    // Best fit is prior to first fit.
    // Check best fit initially. If fails, apply first fit.
    while (metadata) {
    ...
    // For best-fit case: remove best-fit slot from free list
    if (best_fit_flag == 1) {
        void* ptr = metadata + 1;
        simple_remove_from_free_list(metadata, prev);
        return ptr;
    }
    // For first-fit case: continue with first-fit slot
    else {
